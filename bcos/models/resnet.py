...
class BcosResNet(BcosUtilMixin, nn.Module):
    ...
    def forward(self, x):
        if self.also_return_features or not self.baseline_mode:
            ...
            if self.catkd_mode:
                return dict(output=x, features=dict(cams=cams))
            ...
            avg = self.avgpool(x).squeeze(-1).squeeze(-1)
            x = self.fc(x)
            x = self.avgpool(x)
            x = x.flatten(1)
            x = self.logit_layer(x)

            return dict(output=x, features=dict(feats=[F.relu(f) for f in preact_feats], preact_feats=preact_feats, pooled_feat=avg))
