...
__all__ = [
    ...
    "create_configs_with_different_criterion_params",
    # Global Enums
    "ExpMethod",
]

def safe_replace(source: str, a: str, b: str):
    """
    Similar to .replace() of str class, but asserts that the substring exists
    """
    assert a in source, f'String {a} was not found in {source} to be replaced by {b}'
...
class ExpMethod(Enum):
    BCOS_WEIGHT="BcosWeight"
    GRAD_CAM_POS="GradCamPos"
    ...
    def to_key(self):
        """
        Returns a key string to be for loading files and entries from dicts
        """
        return {
            ExpMethod.BCOS_WEIGHT: 'weight',
            ExpMethod.GRAD_CAM_POS: 'grad_cam',
        }[self]
...
def create_configs_with_different_seeds(
    ...
    for seed in seeds:
        ...
        new_configs = {f"{key}-{seed=}" if seed is not None else key: value for key, value in new_configs.items()}
...
def create_configs_with_different_criterion_params(
    configs: "dict[str, dict[str, Any]]", params_dict, precomputed_paths_dict,
):
    """
    For each experiment config in given configs, changes the exp coef to given coef(s).

    Parameters
    ----------
    configs : dict[str, dict[str, Any]]
        The name to config dict.
    params_dict: dict[str, list[int] | int]
    ...
    """
    shorten_names = dict(
        gt_logit_loss_coef = "GtL",
        teacher_logit_loss_coef = "TeL",
        teacher_logit_temperature = "TeT",
        explanation_loss_coef = "EXP",
        explanation_method = 'Method', 
        exp_of = 'ExpOf',
    )

    result = copy.deepcopy(configs)
    for param_key, param_values in tqdm(params_dict.items()):
        prev_configs = copy.deepcopy(result)
        result = dict() # start fresh, so that we don't have incomplete keys from prev iterations
        for param_value in param_values:
            # Add the name tag
            new_configs = {
                f"{key}-{shorten_names[param_key]}{param_value}": value
                for key, value in prev_configs.items()
            }
            # Now actually change the configs
            for name, config in new_configs.items():
                config["criterion_config"][param_key] = param_value
                if config['test_criterion_config'] is not None:
                    config["test_criterion_config"][param_key] = param_value
        
                ############  Extra Rules for specific updates ############
                match param_key:
                    case 'explanation_loss_coef':
                        # Deduce exp_distill flag
                        config['criterion_config']['explanation_distillation'] = param_value != 0
                        if config['test_criterion_config'] is not None:
                            config["test_criterion_config"]['explanation_distillation'] = param_value != 0

                    case 'explanation_method':
                        # Set the path of where the precomputed stuff are, based on the method being used!
                        # This is important only in the case of Fixed-Teacher distillation.
                        _path = precomputed_paths_dict[param_value]
                        if _path is None:
                            config['data']['precomputed_dir'] = None
                        elif config['criterion_config']['exp_of'] == 'top1':
                            config['data']['precomputed_dir'] = ospj(_path, 'topk_teacher')
                        else:
                            raise NotImplementedError
                        if param_value in [ExpMethod.GRAD_CAM_POS]: # Gradcam always has to get resized
                            config['data']['rescale_maps_to_img'] = True
                    case _:
                        pass
                ######################################
                new_configs[name] = copy.deepcopy(config) #Update the config
            result.update(new_configs)
    return result