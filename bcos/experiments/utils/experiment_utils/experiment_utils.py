...
from bcos.data.datamodules import PreComputedDataModule, FewshotDataModule
__all__ = [
    "Experiment",  # main API
]
...
class Experiment:
    ...
    def get_datamodule(self, **data_config_overrides):
        ...
        is_ffkd = self.config.get('ffkd_training', False)
        if is_ffkd:
            datamodule = PreComputedDataModule(datamodule, method=self.config['criterion_config']['explanation_method'])
        
        is_fewshot = self.config.get('fewshot_training', False)
        if is_fewshot:
            datamodule = FewshotDataModule(datamodule,
                        train_samples_per_class=self.config['train_samples_per_class'],
                        nb_classes=self.config['data']['num_classes'], shot_seed=self.config['shot_seed'])
    ...
    def load_trained_model(
        ...
        **kwargs: Any
        ...
        if make_sequential:
            seq_model = torch.nn.Sequential(model)
            seq_model.load_state_dict(state_dict, strict=True)
            model = seq_model[0]
        else:
