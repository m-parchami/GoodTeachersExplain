...
class Metrics(dict):
    ...
    def get_last_epoch_and_accuracy(self) -> Tuple[int, float]:
        last_entry = self[self.VALIDATION_KEY][-1]
        last_epoch = int(last_entry[0])
        last_metric_value = float(last_entry[1])
        return last_epoch, last_metric_value